...
| GET | /api/connections/{id} | Yes | Get connection by ID |
| POST | /api/connections | Yes | Send connection request |
| PUT | /api/connections/{id} | Yes | Accept/reject connection |
...
| PUT | /api/groups/{id}/members/{userId} | Yes | Update member role (admin only) |
...
| DELETE | /api/feed/comments/{commentId} | Yes | Delete comment (author only) |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | /api/admin/dashboard | Yes | Admin dashboard metrics |
| GET | /api/admin/users | Yes | List all users (admin) |
| PUT | /api/admin/users/{id}/suspend | Yes | Suspend user (admin) |
