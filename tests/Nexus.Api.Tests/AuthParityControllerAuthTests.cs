...
/*
 * Auth-gate tests for AuthParityController.
 * Verifies the per-action [Authorize] gate on /api/auth/check-session.
 */
...
public class AuthParityControllerAuthTests : IntegrationTestBase
{
    public AuthParityControllerAuthTests(NexusWebApplicationFactory factory) : base(factory) { }

    private const string Path = "/api/auth/check-session";
