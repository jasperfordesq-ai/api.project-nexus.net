...
public sealed class DirectMessageReactionRouteOwnershipTests
{
    [Fact]
    public void ReactionRoutes_HaveExactlyOneCanonicalOwner()
    {
        ...
        routes.Count(route => route == "POST api/v2/messages/{messageId:int}/reactions").Should().Be(1);
        routes.Count(route => route == "GET api/v2/messages/reactions/batch").Should().Be(1);
        routes.Count(route => route == "GET api/messages/reactions-batch").Should().Be(1);
    }
}
