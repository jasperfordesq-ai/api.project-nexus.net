...
public sealed class EventOfflineCheckinWorkflowParityTests : IntegrationTestBase
{
    ...
    public async Task OfflineSync_IsDurableIdempotentAndConflictResolutionIsAppendOnly()
    {
        ...
        using var scope=Factory.Services.CreateScope();var db=scope.ServiceProvider.GetRequiredService<NexusDbContext>();(await db.EventOfflineSyncDecisions.IgnoreQueryFilters().Where(x=>x.ItemId==itemId).OrderBy(x=>x.DecisionVersion).Select(x=>x.Outcome).ToListAsync()).Should().Equal("conflict","rejected");
    }
