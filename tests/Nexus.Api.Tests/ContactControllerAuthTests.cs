...
/*
 * Auth-gate tests for ContactController.
 * Verifies the per-action [Authorize(Policy = AdminOnly)] gate on /api/admin/contact (ContactController).
 */
...
public class ContactControllerAuthTests : IntegrationTestBase
{
    public ContactControllerAuthTests(NexusWebApplicationFactory factory) : base(factory) { }

    private const string Path = "/api/admin/contact";
