...
/*
 * Auth-gate tests for AdminFeedController (admin moderation of social feed).
...
public class AdminFeedControllerAuthTests : IntegrationTestBase
{
    public AdminFeedControllerAuthTests(NexusWebApplicationFactory factory) : base(factory) { }

    private const string Path = "/api/admin/feed/stats";

    [Theory]
    [InlineData("anonymous", (int)HttpStatusCode.Unauthorized)]
    [InlineData("member", (int)HttpStatusCode.Forbidden)]
    [InlineData("admin", 200)]
    public async Task AdminFeedStats_AuthGate(string role, int expectedStatus)
    {
        if (role == "anonymous")
        {
            ClearAuthToken();
        ...
        if (role == "admin")
        {
            // Admin must not be rejected by auth pipeline (anything <401 proves
            // auth passed; downstream may return 200/400/500).
