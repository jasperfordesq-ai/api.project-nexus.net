...
/*
 * Auth-gate tests for AdminDiagnosticsController (deploy-ground-truth view).
...
public class AdminDiagnosticsControllerAuthTests : IntegrationTestBase
{
    public AdminDiagnosticsControllerAuthTests(NexusWebApplicationFactory factory) : base(factory) { }

    private const string Path = "/api/admin/system/diagnostics/probe";

    [Theory]
    [InlineData("anonymous", (int)HttpStatusCode.Unauthorized)]
    [InlineData("member", (int)HttpStatusCode.Forbidden)]
    [InlineData("admin", 200)]
    public async Task AdminDiagnosticsProbe_AuthGate(string role, int expectedStatus)
    {
        if (role == "anonymous")
        {
            ClearAuthToken();
