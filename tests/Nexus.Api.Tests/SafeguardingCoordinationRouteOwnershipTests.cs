...
public sealed class SafeguardingCoordinationRouteOwnershipTests
{
    [Fact]
    public void CoordinatorAssistanceRoutes_HaveOneOwnerPerCanonicalTemplate()
    {
        ...
        routes.Count(route => route == "POST api/v2/messages/{id:int}/request-coordinator")
            .Should().Be(1);
        routes.Count(route => route == "POST api/messages/{id:int}/request-coordinator")
            .Should().Be(1);
    }
}
