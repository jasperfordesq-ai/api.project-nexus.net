...
| Marketplace / commerce | 244 matched files including `Marketplace*Controller.php`, `MerchantOnboardingController.php`, `MerchantOnboardingService.php`, `MarketplacePromotionController.php`, `MarketplaceCommunityDeliveryController.php`, `MarketplacePickupSlotController.php`, `Marketplace*Service.php`, marketplace models, merchant/coupon/ads routes, Laravel React marketplace pages and components | 10 matched files including `MarketplaceController.cs`, `MarketplaceService.cs`, `MarketplaceEntities.cs`; `CaringCommunityMarktController.cs` now aggregates marketplace items into the caring-community feed; `MemberParityController.cs` now covers the Laravel React merchant-onboarding status/step/complete contract with focused tests; `LaravelReactFrontendContractController.cs` now returns React-shaped buyer shipping options for `/api/v2/marketplace/sellers/{sellerId}/shipping-options`; `MarketplaceController.cs` now covers the Laravel React listing browse/detail/save/report contract, promotion-selector products/promote-submit, marketplace offer-create/list/action, seller coupon CRUD, BuyNow order/payment-intent, order history/sales/purchases plus ship and confirm-delivery actions, seller pickup-slot create/list/delete, pickup reservation/my-pickups/QR-scan, and community-delivery offer create/list/accept/confirm contracts; `V15SocialCompatibilityController.cs` now covers BuyNow coupon validation | Deep workflow and contract parity still needed across listing create/edit media upload and seller-management workflows, deeper order lifecycle workflows such as refunds/disputes/auto-complete, real Stripe payments, escrow, full pickup order/payment orchestration, coupon redemption/payment side effects, ads, stricter offer action authorization/expiry/notification side effects, persisted community-delivery notes/estimated-minutes/timestamps, and broader merchant workflows |
...
4. **Marketplace / commerce / monetization**
   ...
   - Current evidence: Laravel React merchant-onboarding wizard status,
     ...
     Buyer shipping selector options now return React-compatible fields.
     ...
     Pickup reservation, buyer my-pickups, and seller QR scan calls now return
     Laravel reservation field names and picked-up status transitions.
