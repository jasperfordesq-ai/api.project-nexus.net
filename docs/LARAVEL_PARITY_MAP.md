...
| --- | --- | --- | --- |
| Admin users | `AdminUsersController.php`, Laravel React `admin/api/adminApi.ts`, `admin/modules/users/UserList.tsx`, admin user tests | `AdminCompatibilityController.cs`, `AdminExplicitParityController.cs`, `AdminExplicitParityControllerTests.cs`; bulk approve/suspend now accept Laravel React `user_ids`/`reason` payloads, tenant-scope eligible users, update activation/suspension state, skip acting/admin/out-of-tenant/missing users, and return shared `success`/`failed`/`skipped_ids` envelopes | Deeper admin-user list/detail/update schema parity, CSV import/export, welcome-credit/email/in-app notification side effects, impersonation/session parity, and audit-log detail still need endpoint-level audit |
