...
| Marketplace / commerce | 244 matched files including `Marketplace*Controller.php`, `MerchantOnboardingController.php`, `MerchantOnboardingService.php`, `MarketplacePromotionController.php`, `MarketplaceCommunityDeliveryController.php`, `Marketplace*Service.php`, marketplace models, merchant/coupon/ads routes, Laravel React marketplace pages and components | 10 matched files including `MarketplaceController.cs`, `MarketplaceService.cs`, `MarketplaceEntities.cs`; `CaringCommunityMarktController.cs` now aggregates marketplace items into the caring-community feed; `MemberParityController.cs` now covers the Laravel React merchant-onboarding status/step/complete contract with focused tests; `LaravelReactFrontendContractController.cs` now returns React-shaped buyer shipping options for `/api/v2/marketplace/sellers/{sellerId}/shipping-options`; `MarketplaceController.cs` now covers the Laravel React promotion-selector products/promote-submit, marketplace offer-create/list/action, and community-delivery offer create/list/accept/confirm contracts | Deep workflow and contract parity still needed across listings, orders, payments, escrow, pickup, coupons, ads, stricter offer action authorization/expiry/notification side effects, persisted community-delivery notes/estimated-minutes/timestamps, and broader merchant workflows |
...
4. **Marketplace / commerce / monetization**
   ...
   - Current evidence: Laravel React merchant-onboarding wizard status,
     ...
     listing/counterparty fields, and Laravel action statuses. Community
     delivery offer create/list/accept/confirm calls now use React payloads and
     Laravel response field names/statuses. Donation checkout payment-intent
     and receipt calls now return the React-compatible `client_secret`,
