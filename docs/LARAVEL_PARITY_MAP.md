...
| Migrations | 318 | 87 EF migration classes excluding designers/snapshot |
| OpenAPI operations | 891 | 3,389 static controller operations from parity script |
| Schema tables | 361 Laravel source tables | 310 .NET static table names |
...
operations after supplemental API route parsing and de-duplication, 2,244 static
matches, and 185 missing operations. Laravel `govuk-alpha*` accessible page
...
source tables, 310 .NET table names, 120 exact matches, 241 missing Laravel-side
...
`artifacts/parity/backlog/parity-backlog.json` on 2026-07-05 with 6,678 open
