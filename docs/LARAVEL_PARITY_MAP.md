...
4. **Marketplace / commerce / monetization**
   ...
   - Current evidence: Laravel React merchant-onboarding wizard status,
     ...
     `promotion_type` and Laravel-style success/data envelopes. Donation
     checkout payment-intent and receipt calls now return the React-compatible
     `client_secret`, `donation_id`, and receipt JSON envelopes.
