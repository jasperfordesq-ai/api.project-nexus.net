...
| Legacy social comments | `SocialController::comments/fetchComments/submitComment`, `CommentService`, `MentionService`, `SocialNotificationService` | `V15SocialCompatibilityController.Comments`, `ThreadedComment`, `ContentMention`, `SocialCommentContactPolicy`, `20260714234546_SocialCommentMentionParity`, and focused unit/PostgreSQL tests; `/api/social/comments` accepts Laravel legacy `action=fetch|submit` payloads, persists tenant-bound mentions, applies the shared safeguarding boundary, and creates recipient-locale mention/reply/content-owner notifications before non-critical push fan-out. Canonical mention search now returns the tenant-unique public username instead of email | Exact sanitizer parity, broader target workflow tests, provider-backed push proof, and browser smoke coverage remain gaps |
