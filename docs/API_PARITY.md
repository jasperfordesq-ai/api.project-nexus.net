...
| .NET static controller operations | 4,316 | Generated by `scripts/compare-laravel-api-parity.ps1`; not a parity score. |
| Script-generated source operations | 2,449 | Laravel OpenAPI plus de-duplicated supplemental static API route declarations; Laravel `govuk-alpha*` accessible page routes are excluded. |
| Script-generated matched operations | 2,438 | Static method/path or method/shape matches. |
| Script-generated missing operations | 11 | Live 2026-07-12 regeneration after the safeguarding slice; runtime contract verification remains endpoint/workflow-specific. |

The current live delta is the seven document-era admin vetting writes
`POST /api/admin/vetting`, `PUT|DELETE /api/admin/vetting/{id}`,
`POST /api/admin/vetting/{id}/upload|verify|reject`, and
`POST /api/admin/vetting/bulk`, plus `GET /api/pwa/manifest`,
`POST /api/admin/prerender/reset-all`, `POST /api/prerender/invalidate`, and
`POST /api/volunteering/guardian-consents/verify/{token}`. The document-era
vetting routes are deliberately unresolved until legacy evidence disposition is
safe; they must never authorize contact. Route closure would still not prove
the workflows.
...
Three forward-only migrations extend the recorded runtime chain to 114, latest
`20260712023810_SafeguardingPreferenceDependencyParity`. Migration 112 creates
...
A blank PostgreSQL replay applied all 114 migrations. A valid populated
...
114 schema. Exact catalog containment and `has-pending-model-changes` are green.
...
least one and runs the normal message effects. Provider transcription and exact
Laravel message-column storage remain open.

This checkpoint is not direct-message completion. The exact residual P0/P1
queue is: sender-only 24-hour edit with `body`/sanitization/policy/edited
metadata; participant-safe scoped message delete; partner-ID per-user archive,
archived inbox, unread filtering, and real restore; durable allow-listed
message reactions and batch aggregation; typing preflight plus the canonical
Pusher channel/event; and server-authoritative coordinator help with
`SAFEGUARDING_NOT_RESTRICTED`, staff delivery, dedupe, and audit. Existing
compatibility handlers for those operations remain false, destructive, or
incomplete and must not be certified from route-only tests.

The final consolidated affected regression selected and passed 323/323 with
zero failed or skipped in 20m47s. This is not a full-suite pass, CI green,
unchanged-frontend runtime proof, or 1000/1000 claim.
...
2026-07-12 live inventory supersedes it with 2,438/2,449 matched and the 11
...
API process and all nine `/api/webauthn/*` routes have one real owner. Both
challenge stores remain process-local; trusted devices, security notifications,
TOTP-specific encryption-key separation, multi-node proof, WebAuthn
...
The Laravel React message reaction route has shallow compatibility coverage for
`POST /api/v2/messages/{id}/reactions`, but remains P1. It stores the toggle in
tenant configuration instead of Laravel's `message_reactions`, accepts no exact
six-emoji allowlist, does not re-evaluate contact policy before an add, and has
no durable aggregate/batch projection. `GET
/api/v2/messages/reactions/batch?ids=...` still returns an empty placeholder.
Removal after a policy closes, tenant/participant isolation, the exact
`errors[]` contract, and reload behavior all require focused proof.
