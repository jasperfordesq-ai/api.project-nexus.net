...
| Surface | Score | Meaning |
| --- | ---: | --- |
| ASP.NET static API method/path inventory | 965/1000 | 2,500 of 2,592 current Laravel operations matched, with 92 route-shape gaps after the three-route Event Staff aggregate closed; this remains route-shape coverage rather than behavioral parity |
...
| --- | --- |
| ASP.NET static operations | 4,420 |
| Laravel source operations | 2,592 |
| Static method/path matches | 2,500 matched, 92 missing |
...
| Schema inventory | Live: 377 Laravel migration files, 128 ASP.NET migration source files, 455 Laravel source tables, 364 ASP.NET tables, 173 exact matches, 282 missing Laravel names, and 191 ASP.NET-only names. EF applied 126 migrations in the latest blank replay. |
...
| Migration runtime chain | EF applied all 126 migrations through `20260712232721_EventStaffWorkflowParity` to blank disposable PostgreSQL, and `has-pending-model-changes` is green. The latest migration adds exact staff assignment/history tables, closed role/status/version constraints, and two database history-immutability triggers. Focused owner grant/list/revoke/replay, contained co-organizer delegation, tenant/idempotency boundaries, and history/outbox proof passed 4/4. Prior agenda, Event People, registration/waitlist, broadcast/template/lifecycle, and retained migration evidence remains green. No production resource was touched. |
