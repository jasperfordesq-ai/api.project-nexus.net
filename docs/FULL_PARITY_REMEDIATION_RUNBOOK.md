...
| Surface | Score | Meaning |
| --- | ---: | --- |
| ASP.NET static API method/path inventory | 950/1000 | 2,453 of 2,583 current Laravel operations matched, with 130 route-shape gaps after the expanded supplemental inventory; this is route-shape coverage, not behavioral parity |
...
| --- | --- |
| ASP.NET static operations | 4,339 |
| Laravel source operations | 2,583 |
| Static method/path matches | 2,453 matched, 130 missing |
...
| Schema inventory | Live: 373 Laravel migration files, 121 ASP.NET migration source files, 455 Laravel source tables, 340 ASP.NET tables, 149 exact matches, 306 missing Laravel names, and 191 ASP.NET-only names. EF applied 119 migrations in the latest blank replay. |
...
| Migration runtime chain | EF applied all 119 migrations through `20260712163203_GroupInviteAndExportLifecycleParity` to blank disposable PostgreSQL, and `has-pending-model-changes` is green. The latest migration preserves existing memberships as active, derives link/email invite types, initializes invite update timestamps, rebuilds cached group member counts, and adds the queued export table and indexes. Prior group form/Q&A, populated, and invalid-migration evidence remains retained. No production resource was touched. |
