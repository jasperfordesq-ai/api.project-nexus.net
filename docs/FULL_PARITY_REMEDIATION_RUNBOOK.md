...
| Surface | Score | Meaning |
| --- | ---: | --- |
| ASP.NET static API method/path inventory | 948/1000 | 2,449 of 2,583 current Laravel operations matched, with 134 route-shape gaps after the expanded supplemental inventory; this is route-shape coverage, not behavioral parity |
...
| --- | --- |
| ASP.NET static operations | 4,329 |
| Laravel source operations | 2,583 |
| Static method/path matches | 2,449 matched, 134 missing |
...
| Schema inventory | Live: 373 Laravel migration files, 120 ASP.NET migration source files, 455 Laravel source tables, 339 ASP.NET tables, 148 exact matches, 307 missing Laravel names, and 191 ASP.NET-only names. EF applied 118 migrations in the latest blank replay. |
...
| Migration runtime chain | EF applied all 118 migrations through `20260712152645_GroupFormLifecycleParity` to blank disposable PostgreSQL. The form migration preserved existing privacy as visibility, retained every existing group as active, and installed group editor columns plus tenant-scoped type/template tables. Prior Q&A, populated, and invalid-migration evidence remains retained. No production resource was touched. |
