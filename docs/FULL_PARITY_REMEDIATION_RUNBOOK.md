...
| Surface | Score | Meaning |
| --- | ---: | --- |
| ASP.NET static API method/path inventory | 952/1000 | 2,460 of 2,583 current Laravel operations matched, with 123 route-shape gaps after the expanded supplemental inventory; this is route-shape coverage, not behavioral parity |
...
| --- | --- |
| ASP.NET static operations | 4,346 |
| Laravel source operations | 2,583 |
| Static method/path matches | 2,460 matched, 123 missing |
...
| Schema inventory | Live: 373 Laravel migration files, 122 ASP.NET migration source files, 455 Laravel source tables, 342 ASP.NET tables, 151 exact matches, 304 missing Laravel names, and 191 ASP.NET-only names. EF applied 120 migrations in the latest blank replay. |
...
| Migration runtime chain | EF applied all 120 migrations through `20260712175611_EventLifecycleParity` to blank disposable PostgreSQL, and `has-pending-model-changes` is green. The latest migration safely backfills canonical dual-axis event state, adds reminder closure state plus immutable event history and transactional outbox tables, and installs PostgreSQL history mutation guards. Prior group invite/export/form/Q&A, populated, and invalid-migration evidence remains retained. No production resource was touched. |
