...
| Surface | Score | Meaning |
| --- | ---: | --- |
| ASP.NET static API method/path inventory | 963/1000 | 2,497 of 2,592 current Laravel operations matched, with 95 route-shape gaps after the seven-route Event Agenda aggregate closed; this remains route-shape coverage rather than behavioral parity |
...
| --- | --- |
| ASP.NET static operations | 4,417 |
| Laravel source operations | 2,592 |
| Static method/path matches | 2,497 matched, 95 missing |
...
| Schema inventory | Live: 377 Laravel migration files, 127 ASP.NET migration source files, 455 Laravel source tables, 362 ASP.NET tables, 171 exact matches, 284 missing Laravel names, and 191 ASP.NET-only names. EF applied 125 migrations in the latest blank replay. |
...
| Migration runtime chain | EF applied all 125 migrations through `20260712224838_EventAgendaWorkflowParity` to blank disposable PostgreSQL, and `has-pending-model-changes` is green. The latest migration adds the six exact agenda/session tables, optimistic aggregate and registration versions, protected resources, capacity and enum checks, plus three database immutability/no-delete triggers. Focused owner lifecycle/replay, member visibility/registration, conflict/tenant, and durable history proof passed 4/4. Prior Event People, registration/waitlist, broadcast/template/lifecycle, and retained migration evidence remains green. No production resource was touched. |
