...
| Source | Count | Notes |
| --- | ---: | --- |
| Laravel migrations | 317 | PHP migration files under `database/migrations`. |
| ASP.NET EF migrations | 84 | Committed EF migration classes, excluding `.Designer.cs` and model snapshot files. |
...
| ASP.NET tables | 308 | Union of EF `ToTable(...)`, `[Table(...)]`, and migration `CreateTable(...)` names. |
| Exact matched tables | 118 | Static name matches only. |
| Missing Laravel tables | 243 | Laravel source tables with no exact .NET table name. |
...
| `verein_*` | 6 | Verein/Clubs needs real domain schema and workflow parity. |
