...
| JWT metadata | `Jwt__Issuer`, `Jwt__Audience`, `Jwt__AccessTokenExpiryMinutes` | Empty issuer/audience changes validation behavior; verify the client contract. |
...
| Message broker | `RabbitMq__Host`, `Port`, `Username`, `Password`, `VirtualHost`, `ExchangeName`, `Enabled` | The current binder does not use a single `RabbitMq__Uri` key. |
...
- `.env.production.example` is a key-name reference, not a deployment recipe
  and not a file automatically loaded by every Compose command.
