...
test.describe('representative authenticated-page accessibility gate', () => {
  ...
  test('Arabic wallet management preserves Laravel catalog output with RTL reflow', async ({ browser, baseURL }, testInfo) => {
    const context = await browser.newContext({ baseURL, storageState });
    const page = await context.newPage();
    await page.setViewportSize({ width: 320, height: 640 });

    try {
      const path = `${authenticatedMountPath}/wallet/manage?locale=ar`;
      const response = await page.goto(path, { waitUntil: 'domcontentloaded' });
      expect(response, `${path} did not return a document response`).not.toBeNull();
      expect(response.status(), `${path} returned HTTP ${response.status()}`).toBeLessThan(400);
      expect(response.headers()['content-language']).toBe('ar');
      ...
      await expect(page.locator('html')).toHaveAttribute('lang', 'ar');
      await expect(page.locator('html')).toHaveAttribute('dir', 'rtl');
      await expect(page.locator('h1')).toHaveText(translate('ar', 'govuk_alpha_wallet.manage.title'));
      await expect(page.locator('.govuk-body-l')).toHaveText(translate('ar', 'govuk_alpha_wallet.manage.description'));
      await expect(page.locator('#donate')).toContainText(translate('ar', 'govuk_alpha_wallet.donate.warning'));
      ...
      const axeResults = await new AxeBuilder({ page }).analyze();
      await testInfo.attach('authenticated-arabic-wallet-manage', {
        body: Buffer.from(JSON.stringify({
          url: page.url(),
          viewport: { width: 320, height: 640 },
          overflow,
