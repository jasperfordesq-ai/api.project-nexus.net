...
test.describe('representative authenticated-page accessibility gate', () => {
  ...
  test('Arabic member discovery preserves Laravel catalog output with RTL reflow', async ({ browser, baseURL }, testInfo) => {
    const context = await browser.newContext({ baseURL, storageState });
    const page = await context.newPage();
    await page.setViewportSize({ width: 320, height: 640 });
    const path = `${authenticatedMountPath}/members/discover?locale=ar`;
    ...
    try {
      ...
      await expect(page.locator('html')).toHaveAttribute('lang', 'ar');
      await expect(page.locator('html')).toHaveAttribute('dir', 'rtl');
      await expect(page.locator('h1')).toHaveText(translate('ar', 'govuk_alpha_members.discover.heading'));
      await expect(page.locator('.govuk-body-l')).toHaveText(translate('ar', 'govuk_alpha_members.discover.description'));
      ...
      const axeResults = await new AxeBuilder({ page }).analyze();
      await testInfo.attach('authenticated-arabic-members-discover', {
        body: Buffer.from(JSON.stringify({
          url: page.url(),
          viewport: { width: 320, height: 640 },
          overflow,
