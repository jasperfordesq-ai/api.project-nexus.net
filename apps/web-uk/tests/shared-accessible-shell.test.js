...
describe('shared accessible frontend shell', () => {
  ...
  it('renders the Laravel Blade-style AI assistant page when signed in', async () => {
    const cookieSignature = require('cookie-signature');
    const api = require('../src/lib/api');
    api.getAiConversations.mockClear();
    api.getAiConversation.mockClear();
    api.getAiChatStarters.mockClear();
    api.getAiProviders.mockClear();
    api.getAiLimits.mockClear();
    api.getAiConversations.mockResolvedValueOnce({
      data: [
        { id: 12, title: 'Gardening help', updated_at: '2026-07-08T10:00:00Z' }
      ]
    });
    api.getAiConversation.mockResolvedValueOnce({
      data: {
        id: 12,
        messages: [
          { id: 30, role: 'user', content: 'How do I find a gardener?', created_at: '2026-07-08T10:00:00Z' },
          { id: 31, role: 'assistant', content: 'Search listings and filter by gardening.', created_at: '2026-07-08T10:00:02Z' }
        ]
      }
    });
    api.getAiChatStarters.mockResolvedValueOnce({
      data: { starters: ['How do I find volunteering near me?'] }
    });
    api.getAiProviders.mockResolvedValueOnce({
      data: { enabled: true, default: 'gemini', providers: ['gemini'] }
    });
    api.getAiLimits.mockResolvedValueOnce({
      data: { limits: { allowed: true, remaining: 29 } }
    });
    const signedToken = `s:${cookieSignature.sign('test-token', process.env.COOKIE_SECRET)}`;
    ...
    expect(api.getAiConversation).toHaveBeenCalledWith('test-token', '12');
    expect(api.getAiChatStarters).toHaveBeenCalledWith('test-token');
    expect(response.status).toBe(200);
    expect(response.text).toContain('Community help');
    expect(response.text).toContain('AI assistant');
    expect(response.text).toContain('Ask a question and the community assistant will reply.');
    expect(response.text).toContain('Replies are generated by AI and may be inaccurate.');
    expect(response.text).toContain('Your conversations');
    expect(response.text).toContain('Gardening help');
    expect(response.text).toContain('Conversation');
    expect(response.text).toContain('How do I find a gardener?');
    expect(response.text).toContain('Search listings and filter by gardening.');
    expect(response.text).toContain('Your message');
    expect(response.text).toContain('action="/chat"');
    expect(response.text).not.toContain('shared accessible frontend preparation page');
    expect(response.text).not.toContain('AI assistant pages will be wired');
  });

  it('posts AI assistant messages through the Laravel chat API and redirects to the thread', async () => {
    const cookieSignature = require('cookie-signature');
    const api = require('../src/lib/api');
    api.sendAiChatMessage.mockClear();
    api.getAiConversations.mockResolvedValueOnce({ data: [] });
    api.getAiChatStarters.mockResolvedValueOnce({ data: { starters: [] } });
    api.getAiProviders.mockResolvedValueOnce({ data: { enabled: true } });
    api.getAiLimits.mockResolvedValueOnce({ data: { limits: { allowed: true } } });
    api.sendAiChatMessage.mockResolvedValueOnce({
      data: {
        conversation_id: 44,
        message: { id: 88, role: 'assistant', content: 'Start with the listings directory.' }
      }
    });
    const signedToken = `s:${cookieSignature.sign('test-token', process.env.COOKIE_SECRET)}`;
    const agent = request.agent(app);
    ...
    expect(api.sendAiChatMessage).toHaveBeenCalledWith('test-token', {
      conversation_id: 12,
      message: 'How do I find help?'
    });
    expect(response.status).toBe(302);
    expect(response.headers.location).toBe('/chat?c=44&status=sent');
  });
