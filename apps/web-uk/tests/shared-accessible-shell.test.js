...
describe('shared accessible frontend shell', () => {
  ...
  it('renders the Blade-style NEXUS score unavailable state under the tenant alpha route prefix', async () => {
    const response = await request(app).get('/acme/alpha/nexus-score');

    expect(staticPageRoutes.pages['/nexus-score']).toBeUndefined();
    expect(response.status).toBe(200);
    expect(response.text).toContain('NEXUS score');
    expect(response.text).toContain('Your reputation score, based on how you take part in the community.');
    expect(response.text).toContain('Related NEXUS score pages');
    expect(response.text).toContain('href="/acme/alpha/nexus-score/tiers"');
    expect(response.text).toContain('Score tiers');
    expect(response.text).toContain('Your score is not available yet. Take part in the community and check back soon.');
    expect(response.text).not.toContain('shared accessible frontend preparation page');
  });
  ...
  it('renders the Blade-style organisations directory and registration form under the tenant alpha route prefix', async () => {
    const response = await request(app).get('/acme/alpha/organisations?q=club');
    ...
    expect(response.text).toContain('href="/acme/alpha/organisations/browse"');
    expect(response.text).toContain('Browse all organisations');
    expect(response.text).toContain('href="/acme/alpha/organisations/register"');
    expect(response.text).toContain('Register an organisation');
    expect(response.text).toContain('href="/acme/alpha/organisations/manage"');
    expect(response.text).toContain('Manage my organisations');
    expect(response.text).toContain('action="/acme/alpha/organisations"');
  ...
  it('documents route matrix and backend-switching preparation without readiness claims', () => {
    ...
    expect(routeInventory).toContain('| ASP.NET static skeleton paths | 19 |');
    ...
    expect(routeInventory).toContain('| GET | /organisations | /{tenantSlug}/alpha/organisations | /organisations | organisations.index | organisations | organisations | candidate-route |');
    expect(routeInventory).toContain('| GET | /organisations/browse | /{tenantSlug}/alpha/organisations/browse | /organisations/browse | organisations.browse | organisationsBrowse | organisations | missing |');
    ...
    expect(viewInventory).toContain('| organisations.blade.php | organisations | candidate-exact-view |');
    expect(viewInventory).toContain('| organisations-browse.blade.php | organisations | missing-nunjucks-view |');
