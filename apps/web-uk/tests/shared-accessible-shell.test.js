...
describe('shared accessible frontend shell', () => {
  ...
  it('renders the Laravel-backed Explore hub with live discovery sections', async () => {
    ...
    const mountedUnsigned = await request(app).get('/acme/accessible/explore');
  ...
  it('renders the Laravel-style activity insights page', async () => {
    const cookieSignature = require('cookie-signature');
    const api = require('../src/lib/api');
    const signedToken = `s:${cookieSignature.sign('test-token', process.env.COOKIE_SECRET)}`;
    ...
    expect(signed.text).toContain('A detailed view of your contributions, skills and recent activity across the community.');
    expect(signed.text).toContain('Hours given');
    expect(signed.text).toContain('12.5');
    expect(signed.text).toContain('Hours received');
    expect(signed.text).toContain('5.0');
    expect(signed.text).toContain('Connections');
    expect(signed.text).toContain('4');
    expect(signed.text).toContain('Exchanges');
    expect(signed.text).toContain('5');
    expect(signed.text).toContain('Monthly activity');
    expect(signed.text).toContain('Hours given and received over the last 12 months.');
    expect(signed.text).toContain('March 2026');
    expect(signed.text).toContain('Given 3.5');
    expect(signed.text).toContain('Received 1.0');
    expect(signed.text).toContain('Recent activity');
    expect(signed.text).toContain('Gave hours');
    expect(signed.text).toContain('Helped Grace with a repair cafe booking');
    expect(signed.text).toContain('Connection');
    expect(signed.text).toContain('Joined Community Makers');
    expect(signed.text).toContain('Quick stats');
    expect(signed.text).toContain('Posts (last 30 days)');
    expect(signed.text).toContain('Likes received (last 30 days)');
    expect(signed.text).toContain('+7.5 hours');
    expect(signed.text).toContain('My skills');
    expect(signed.text).toContain('1 offered, 1 requested.');
    expect(signed.text).toContain('Repair cafes');
    expect(signed.text).toContain('Offering');
    expect(signed.text).toContain('Endorsed 2 times');
    expect(signed.text).toContain('Bike maintenance');
    ...
    api.callGoalApi
      ...
      .mockResolvedValueOnce({
        data: [
          {
            id: 42,
            title: 'Support repairs training',
            current_value: 3,
            target_value: 6,
            status: 'active',
            user: {
  ...
  it('keeps empty Laravel AI chat redirects inside the shared tenant mount', async () => {
    const api = require('../src/lib/api');
    const cookieSignature = require('cookie-signature');
    const signedToken = `s:${cookieSignature.sign('test-token', process.env.COOKIE_SECRET)}`;
    const agent = request.agent(app);
    ...
    expect(response.status).toBe(302);
    expect(response.headers.location).toBe('/acme/accessible/chat?status=empty');
    expect(api.sendAiChat).not.toHaveBeenCalled();
  });
  ...
  it('renders the public Laravel feed hashtag detail page with post cards', async () => {
    ...
    expect(response.text).toContain('#repair');
    expect(response.text).toContain('2 posts');
    expect(response.text).toContain('id="feed-item-post-42"');
  ...
  it('keeps signed-out Laravel poll action redirects inside the shared accessible mount', async () => {
    const api = require('../src/lib/api');
    const agent = request.agent(app);
    const first = await agent.get('/acme/accessible/contact');
    const csrfMatch = first.text.match(/name="_csrf" value="([^"]+)"/);
    ...
    const response = await agent
      .post('/acme/accessible/polls/42/vote')
      .type('form')
      .send({
        _csrf: csrfMatch[1],
        option_id: '7'
  ...
  it('renders the Laravel-backed ideation challenge create form', async () => {
      ...
      .mockResolvedValueOnce({
        data: {
          data: [
            {
              id: 9,
              title: 'Community design sprint',
              description: 'Use this template to gather practical neighbourhood ideas.'
            }
          ]
        }
      });
    ...
    const response = await request(app)
      .get('/ideation/new?status=challenge-invalid')
      .set('Cookie', signedCookieHeader());

    expect(response.status).toBe(200);
    expect(response.text).toContain('href="/ideation"');
    expect(response.text).toContain('Ideation');
    expect(response.text).toContain('Create challenge');
    expect(response.text).toContain('Describe the challenge you want people to respond to.');
    expect(response.text).toContain('There is a problem');
    expect(response.text).toContain('Enter a challenge title.');
    expect(response.text).toContain('Enter a challenge description.');
    expect(response.text).toContain('Community design sprint');
    expect(response.text).toContain('Use this template to gather practical neighbourhood ideas.');
    expect(response.text).toContain('Public spaces');
    expect(response.text).toContain('Transport');
    expect(response.text).toContain('name="challenge_status"');
    expect(response.text).toContain('value="draft" checked');
    expect(response.text).toContain('name="max_ideas_per_user"');
    expect(response.text).toContain('name="cover_image"');
    expect(response.text).toContain('name="tags"');
    expect(response.text).toContain('Create challenge');
    expect(response.text).not.toContain('shared accessible frontend preparation page');
  ...
  it('renders the Laravel-backed ideation challenge edit form', async () => {
      ...
      .mockResolvedValueOnce({
        data: {
          data: {
            ...
            description: 'Help make the central park safer after dark.',
            status: 'open',
            category_id: 3,
            category: 'Public spaces',
            prize_description: 'Winning ideas go to the neighbourhood board.',
            submission_deadline: '2026-08-31',
            voting_deadline: '2026-09-15',
            max_ideas_per_user: 2,
    ...
    const response = await request(app)
      .get('/ideation/7/edit?status=challenge-invalid')
      .set('Cookie', signedCookieHeader());

    expect(response.status).toBe(200);
    expect(response.text).toContain('href="/ideation/7"');
    expect(response.text).toContain('Ideation');
    expect(response.text).toContain('Edit challenge');
    expect(response.text).toContain('Update the challenge details and lifecycle state.');
    expect(response.text).toContain('Improve park lighting');
    expect(response.text).toContain('Help make the central park safer after dark.');
    expect(response.text).toContain('Public spaces');
    expect(response.text).toContain('value="3" selected');
    expect(response.text).toContain('Winning ideas go to the neighbourhood board.');
    expect(response.text).toContain('2026-08-31');
    expect(response.text).toContain('2026-09-15');
    expect(response.text).toContain('value="2"');
    expect(response.text).toContain('https://example.test/park.jpg');
    expect(response.text).toContain('parks, safety');
    expect(response.text).toContain('value="open" checked');
    expect(response.text).toContain('Save challenge');
    expect(response.text).not.toContain('shared accessible frontend preparation page');
  ...
  it('keeps Laravel group exchange validation redirects inside the shared tenant mount', async () => {
    const cookieSignature = require('cookie-signature');
    const signedToken = `s:${cookieSignature.sign('test-token', process.env.COOKIE_SECRET)}`;
    const agent = request.agent(app);

    const first = await agent
      .get('/acme/accessible/group-exchanges/new')
      .set('Cookie', `token=${encodeURIComponent(signedToken)}`);
    const csrfMatch = first.text.match(/name="_csrf" value="([^"]+)"/);

    expect(first.status).toBe(200);
    expect(csrfMatch).not.toBeNull();
    expect(first.text).toContain('action="/acme/accessible/group-exchanges/new"');

    const response = await agent
      .post('/acme/accessible/group-exchanges/new')
      .set('Cookie', `token=${encodeURIComponent(signedToken)}`)
      .type('form')
      .send({
        _csrf: csrfMatch[1],
    ...
    expect(response.status).toBe(302);
    expect(response.headers.location).toBe('/acme/accessible/group-exchanges/new?status=create-invalid');
  });
