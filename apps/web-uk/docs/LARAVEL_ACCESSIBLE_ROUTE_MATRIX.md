...
| Jobs | `/jobs`, `/jobs/{id}` plus depth POST workflows | `/jobs`, `/jobs/:id` plus matching depth POST aliases | Partial Laravel-backed candidate: browse and detail GET pages require signed auth, call `/api/v2/jobs` with Laravel `q`, `type`, `commitment`, `sort`, `remote`, and `offset` filter mapping and `/api/v2/jobs/{id}` for detail, and render Blade-style tabs, filters, result cards, save, and apply forms. POST aliases cover create/update/delete/renew, apply, save/unsave, application status/withdrawal, alerts, interview responses, and offer responses through Laravel v2 jobs APIs. Multipart CV proxying, employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Jobs | 0 exact route gaps | none exact | Browse/detail GET pages now call Laravel v2 jobs APIs and render Blade-style browse filters, result cards, save toggles, summary lists, skills, and apply forms with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Deeper jobs GET pages still rely on preparation pages, multipart CV upload proxying is not implemented, and employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
