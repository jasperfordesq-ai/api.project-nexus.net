...
Parent-domain child tenant routing, full template helper conversion, and a full
temporary Web UK process host-root smoke for custom domains are still open.
...
for `timebank.global` and `project-nexus.ie` is green, but a full temporary Web
UK process host-root smoke is still open because the live probe rendered the
shared chooser during this slice.
