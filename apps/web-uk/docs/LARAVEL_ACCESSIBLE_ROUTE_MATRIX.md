...
| Groups | `/groups` plus depth POST workflows | `/groups` plus matching depth POST aliases | Exact Laravel accessible group route declarations are now present locally. Existing GET pages remain local/protected group pages; POST aliases cover invites, notification preferences, image/file safe failure, file delete, announcements, discussions, group feed posts, member actions, and join request decisions through Laravel v2 group/feed APIs. Multipart image/file proxying, Blade depth rendering, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Marketplace | `/marketplace` | `/marketplace` | Partial Laravel-backed candidate: browse, detail, create/edit listing, buy, offer, and report GET pages require signed auth where Laravel does, call `/api/v2/marketplace/listings`, `/api/v2/marketplace/listings/{id}`, and `/api/v2/marketplace/categories`, and render Blade-style filters, category links, listing cards, item summaries, status banners, and listing/buyer/report forms. POST aliases now cover listing create/update/delete/renew, save/unsave, buy, offer, report, offer decisions, order actions, seller onboarding, pickup slots, and seller coupons through Laravel v2 marketplace APIs. Remaining seller dashboards, saved/free/search/category/seller listings, offers/orders/sales, onboarding, pickup slots, coupons, hosted no-JS checkout redirects, media uploads, tenant/feature gates, and runtime behavior are not certified. |
...
| Podcasts | `/podcasts` | `/podcasts` | GET list, detail, episode, studio, create, and manage pages now render Laravel-backed accessible pages through `/api/v2/podcasts` and `/api/v2/podcasts/mine`; POST aliases cover show subscribe, studio show create/update/publish/delete, and episode add/publish/delete through Laravel v2 podcast APIs. |
...
| Messages | 0 | none exact | POST aliases now cover archive/restore, edit/delete/translate message actions, voice upload safe failure, group create/reply/member add/remove, and group message reactions through Laravel v2 message/conversation APIs while preserving Laravel status redirects and anchors. GET conversation/group rendering remains local or generated preparation pages; multipart voice upload, translated-text display, group participant data, tenant feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Groups | 0 exact route gaps | none exact | POST aliases now cover invite link/email/revoke, notification preferences, image/file safe missing-file redirects, file delete, announcement create/update/delete/pin, discussion create/reply, group feed posts, member promote/demote/remove, and join request approve/reject through Laravel v2 group/feed APIs while preserving Laravel status redirects and fragments. Existing GET pages remain local/protected routes or generated preparation pages; multipart image/file proxying, Blade depth rendering, owner/admin authorization depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Marketplace/commerce | 0 | none exact | Browse, detail, create/edit listing, buy, offer, and report GET pages now render Laravel-backed marketplace pages through `/api/v2/marketplace/listings` and `/api/v2/marketplace/categories`; POST aliases cover `/marketplace/create`, listing update/delete/renew/save/unsave/buy/offer/report, offer accept/decline/withdraw, order ship/confirm/cancel/pay/rate, seller onboarding, pickup slot create/update/delete/scan, and seller coupon create/update/delete through Laravel v2 marketplace APIs. Remaining seller dashboards, saved/free/search/category/seller listings, offers/orders/sales, onboarding, pickup slots, and coupons still rely on generated preparation pages; hosted no-JS Stripe checkout is represented by v2 payment-intent creation rather than an external Checkout redirect, address/onboarding depth and media uploads are not implemented, and tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. Courses remain a separate commerce-family gap. |
| Podcasts | 0 | none exact | GET aliases now cover `/podcasts`, `/podcasts/{id}`, `/podcasts/{showId}/episodes/{id}`, `/podcasts/studio`, `/podcasts/studio/new`, and `/podcasts/studio/{id}` with Laravel v2 podcast browse/detail and authored-show APIs. POST aliases cover `/podcasts/{id}/subscribe`, `/podcasts/studio/new`, show update/publish/delete, and episode add/publish/delete through Laravel v2 podcast APIs. Remaining gaps: public Laravel web detail routes are numeric-ID based while the public v2 detail API is slug-oriented, multipart audio upload is still not proxied, tenant podcast-author configuration gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Jobs | 0 exact route gaps | none exact | Browse/detail/saved/applications/mine/create/edit/owner-applicants/analytics/pipeline/qualified/alerts/responses/talent-search/talent-profile/employer-brand/employer-onboarding/bias-audit GET pages now call Laravel-compatible jobs/admin/user APIs where data is needed and render Blade-style browse filters, result cards, saved-opportunity removal, my-application filters, withdrawal forms, owner posting actions, create/edit form fields, summary lists, skills, apply forms, applicant analytics/stage controls, analytics summaries/predictions, pipeline columns, qualification breakdowns, job alert controls, interview invitations, offer response forms, talent search filters/cards, candidate profile summaries, employer profile/review/open-job summaries, employer posting guidance, bias audit filter/report tables, owner CSV export, application CV download, and application history timelines with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Multipart CV upload proxying is not implemented, and tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
