...
| --- | ---: | --- | --- |
| Tenant routing | structural | shared-domain `/{tenantSlug}/alpha`, custom accessible domains | First shared-mount slice implemented: `/{tenantSlug}/accessible` routes through the existing flat Express app, shell/home links are prefixed under the active mount, and legacy `/{tenantSlug}/alpha` redirects to `/accessible`. Custom accessible-domain resolution through Laravel `accessible_domain`, parent-domain child tenant routing, root tenant chooser behavior, and template-wide link/action prefixing remain open and uncertified. |
