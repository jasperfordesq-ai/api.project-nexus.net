...
| ASP.NET `apps/web-uk` | 429 | Express app/router/static-page declarations scanned from local source after shell prep; this includes preparation skeletons, generated Laravel GET fallback pages, and route modules that may not be certified workflows yet. |
| Exact method/path matches | 347 | Static matches only. This does not prove workflow, auth, tenant, API, localization, or visual parity. |
| Missing Laravel routes | 261 | Laravel accessible declarations without an exact local method/path equivalent. These are now primarily POST/state-changing workflows. |
...
| Group exchanges | 6 | ready/confirm/cancel, group exchange participants | GET routes have preparation pages; POST workflows are not certified. Standard exchange action/rating POST aliases are now present but GET/detail runtime parity is not certified. |
...
| Marketplace/commerce | 25 | marketplace, seller, orders, coupons, courses, podcasts | GET routes have preparation pages; seller/buyer POST workflows are not certified. Premium POST route declarations are now present, but premium GET/runtime subscription behavior is not certified. |
...
| Resources/settings | 14 | resources comments/upload/delete, linked accounts, appearance, data rights, insurance, availability | Partial or missing POST workflow coverage. Saved item, appreciation, and saved-search POST aliases are now present, but saved/appreciation GET pages and most resources/settings POST workflows remain generated preparation pages or missing. |
