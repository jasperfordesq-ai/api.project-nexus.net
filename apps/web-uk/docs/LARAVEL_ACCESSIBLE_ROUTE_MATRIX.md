...
| Jobs | `/jobs`, `/jobs/{id}` plus depth POST workflows | `/jobs`, `/jobs/:id` plus matching depth POST aliases | Partial Laravel-backed candidate: browse, detail, saved, my-applications, my-postings, and create GET pages require signed auth where Laravel does, call `/api/v2/jobs`, `/api/v2/jobs/{id}`, `/api/v2/jobs/saved`, `/api/v2/jobs/my-applications`, and `/api/v2/jobs/my-postings` with Laravel-compatible filters/cursors, and render Blade-style tabs, filters, result cards, save/unsave, application status, withdrawal, owner actions, and create/apply forms. POST aliases cover create/update/delete/renew, apply, save/unsave, application status/withdrawal, alerts, interview responses, and offer responses through Laravel v2 jobs APIs. Multipart CV proxying, employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Jobs | 0 exact route gaps | none exact | Browse/detail/saved/applications/mine/create GET pages now call Laravel v2 jobs APIs where data is needed and render Blade-style browse filters, result cards, saved-opportunity removal, my-application filters, withdrawal forms, owner posting actions, create form fields, summary lists, skills, and apply forms with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Remaining jobs GET pages still rely on preparation pages, multipart CV upload proxying is not implemented, and employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
