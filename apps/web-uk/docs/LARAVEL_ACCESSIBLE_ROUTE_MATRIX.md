...
| Jobs | `/jobs`, `/jobs/{id}` plus depth POST workflows | `/jobs`, `/jobs/:id` plus matching depth POST aliases | Partial Laravel-backed candidate: browse, detail, saved, and my-applications GET pages require signed auth, call `/api/v2/jobs`, `/api/v2/jobs/{id}`, `/api/v2/jobs/saved`, and `/api/v2/jobs/my-applications` with Laravel-compatible filters/cursors, and render Blade-style tabs, filters, result cards, save/unsave, application status, withdrawal, and apply forms. POST aliases cover create/update/delete/renew, apply, save/unsave, application status/withdrawal, alerts, interview responses, and offer responses through Laravel v2 jobs APIs. Multipart CV proxying, employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
