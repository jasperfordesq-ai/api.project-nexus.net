...
| Jobs | `/jobs`, `/jobs/{id}` plus depth POST workflows | `/jobs`, `/jobs/:id` plus matching depth POST aliases | Partial Laravel-backed candidate: browse, detail, saved, my-applications, my-postings, create/edit, owner applicants, alerts, and responses GET pages require signed auth where Laravel does, call `/api/v2/jobs`, `/api/v2/jobs/{id}`, `/api/v2/jobs/saved`, `/api/v2/jobs/my-applications`, `/api/v2/jobs/my-postings`, `/api/v2/jobs/{id}/applications`, `/api/v2/jobs/{id}/analytics`, `/api/v2/jobs/alerts`, `/api/v2/jobs/my-interviews`, and `/api/v2/jobs/my-offers` with Laravel-compatible filters/cursors/shapes, and render Blade-style tabs, filters, result cards, save/unsave, application status, withdrawal, owner actions, create/edit/apply forms, applicant stage controls, alert controls, and interview/offer response actions. POST aliases cover create/update/delete/renew, apply, save/unsave, application status/withdrawal, alerts, interview responses, and offer responses through Laravel v2 jobs APIs. Multipart CV proxying, employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
...
| Jobs | 0 exact route gaps | none exact | Browse/detail/saved/applications/mine/create/edit/owner-applicants/alerts/responses GET pages now call Laravel v2 jobs APIs where data is needed and render Blade-style browse filters, result cards, saved-opportunity removal, my-application filters, withdrawal forms, owner posting actions, create/edit form fields, summary lists, skills, apply forms, applicant analytics/stage controls, job alert controls, interview invitations, offer response forms, owner CSV export, and application history timelines with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Remaining jobs GET depth pages still rely on preparation pages, CV download proxying and multipart CV upload proxying are not implemented, and employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
