...
| Jobs | 0 exact route gaps | none exact | Browse/detail/saved/applications/mine/create/edit/owner-applicants/alerts/responses GET pages now call Laravel v2 jobs APIs where data is needed and render Blade-style browse filters, result cards, saved-opportunity removal, my-application filters, withdrawal forms, owner posting actions, create/edit form fields, summary lists, skills, apply forms, applicant analytics/stage controls, job alert controls, interview invitations, and offer response forms with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Remaining jobs GET depth pages still rely on preparation pages, CSV/CV download proxying and multipart CV upload proxying are not implemented, and employer/talent dashboard depth, tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
