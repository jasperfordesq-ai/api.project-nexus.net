...
matches Laravel `TenantContext::getReservedPaths()` exactly; event detail,
...
tests plus scoped Laravel runtime smoke for signed `/exchanges`. This does not
newly certify exchange feature/module gates, participant authorization edges,
localization, broader runtime persistence, or ASP.NET backend compatibility.
Public coupon source templates now also use `urlFor()` for list/detail coupon
links, with source regression coverage, focused render tests, and scoped
...
`TenantContext::getReservedPaths()` exactly, so Laravel-reserved paths such as
