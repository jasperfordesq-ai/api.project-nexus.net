...
| Jobs | 0 exact route gaps | none exact | Browse/detail/saved/applications/mine/create/edit/owner-applicants/analytics/pipeline/qualified/alerts/responses/talent-search/talent-profile GET pages now call Laravel v2 jobs APIs where data is needed and render Blade-style browse filters, result cards, saved-opportunity removal, my-application filters, withdrawal forms, owner posting actions, create/edit form fields, summary lists, skills, apply forms, applicant analytics/stage controls, analytics summaries/predictions, pipeline columns, qualification breakdowns, job alert controls, interview invitations, offer response forms, talent search filters/cards, candidate profile summaries, owner CSV export, application CV download, and application history timelines with signed-out redirects covered by tests. POST aliases cover `/jobs`, job update/delete/renew, apply, save/unsave, application status and withdrawal, alert create/pause/resume/delete, interview accept/decline, and offer accept/reject through Laravel v2 jobs APIs with Laravel status redirects. Remaining employer brand, bias audit, and employer onboarding jobs GET depth pages still rely on preparation pages, multipart CV upload proxying is not implemented, and tenant/feature gates, localization, runtime behavior, and ASP.NET backend compatibility are not certified. |
