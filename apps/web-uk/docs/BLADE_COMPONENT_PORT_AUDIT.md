...
| Auth aliases | `views/forgot-password.blade.php`, `views/reset-password.blade.php`, `views/two-factor.blade.php` | `src/views/forgot-password.njk`, `src/views/reset-password.njk`, `src/views/login.njk`, `src/routes/auth.js` | Partial. Laravel accessible aliases `/login/forgot-password`, `/password/reset`, and `/login/two-factor` now route to local handlers with matching form actions; reset submissions use Laravel's `password` and `password_confirmation` payload. `/login/resend-verification`, localization, full Laravel status-copy parity, tenant-domain routing, and runtime behavior are not certified. |
