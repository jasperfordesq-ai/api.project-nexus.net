 ...
 * Covers: initial state, count fetching, markAsRead, markAllAsRead, unauthenticated cleanup
 *
 * Note: Real-time events are now handled via PusherContext (SignalR), not direct
 * Pusher subscriptions. The NotificationsContext uses usePusherOptional() for
 * real-time message/unread-count updates.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// PusherContext mock (SignalR-based)
// ─────────────────────────────────────────────────────────────────────────────
...
vi.mock('../PusherContext', () => ({
  usePusherOptional: () => ({
    ...
    onUnreadCount: mockOnUnreadCount,
...
describe('NotificationsContext', () => {
  beforeEach(() => {
    ...
    // Default API responses (paths without /v2/ prefix — normalizeEndpoint strips it)
    mockApiGet.mockImplementation((url: string) => {
      if (url === '/notifications/counts') {
  ...
  describe('loading notification counts on mount', () => {
    it('calls /notifications/counts when user is authenticated', async () => {
    ...
    it('uses the messages unread count from /messages/unread-count (not notification count)', async () => {
      const { result } = renderHook(() => useNotifications(), {
        wrapper: notificationsWrapper,
      });

      await waitFor(() => {
        // mockCounts.messages = 2, but /messages/unread-count returns count: 3
  ...
  describe('markAsRead(id)', () => {
    it('decrements unreadCount by 1 after successfully marking a notification read', async () => {
      ...
      expect(mockApiPost).toHaveBeenCalledWith('/notifications/101/read');
    });
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SignalR real-time events (via PusherContext)
  // ─────────────────────────────────────────────────────────────────────────

  describe('SignalR real-time events (via PusherContext)', () => {
    it('subscribes to onNewMessage from PusherContext', async () => {
      renderHook(() => useNotifications(), { wrapper: notificationsWrapper });
    ...
    it('subscribes to onUnreadCount from PusherContext', async () => {
      renderHook(() => useNotifications(), { wrapper: notificationsWrapper });

      await waitFor(() => {
        expect(mockOnUnreadCount).toHaveBeenCalled();
      });
    });
  });
