...
const TRAINING_TYPES = [ { key: 'children_first', label: 'Children First' }, { key: 'vulnerable_adults', label: 'Vulnerable Adults' }, { key: 'first_aid', label: 'First Aid' }, { key: 'manual_handling', label: 'Manual Handling' }, { key: 'other', label: 'Other' } ];
const SEVERITY_OPTIONS = [ { key: 'low', label: 'Low' }, { key: 'medium', label: 'Medium' }, { key: 'high', label: 'High' }, { key: 'critical', label: 'Critical' } ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white" startContent={<Plus className="w-4 h-4" aria-hidden="true" />} onPress={subView === 'training' ? trainingModal.onOpen : incidentModal.onOpen}>
      ...
      {!error && !isLoading && subView === 'training' && (trainings.length === 0 ? (<EmptyState icon={<GraduationCap className="w-12 h-12" aria-hidden="true" />} title={t('safeguarding.no_training_title', 'No training records')} description={t('safeguarding.no_training_desc', 'Add your safeguarding training records to get started.')} action={<Button className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white" onPress={trainingModal.onOpen}>{t('safeguarding.add_training', 'Add Training')}</Button>} />) : (
        <motion.div variants={cV} initial="hidden" animate="visible" className="space-y-3">
          {trainings.map((tr) => (<motion.div key={tr.id} variants={iV}><GlassCard className="p-4"><div className="flex items-start justify-between gap-3"><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><span className="text-sm font-semibold text-theme-primary">{tr.training_name}</span><Chip size="sm" color={trainingStatusColor(tr.status)} variant="flat">{tr.status}</Chip></div><p className="text-xs text-theme-muted">{TRAINING_TYPES.find((tt) => tt.key === tr.training_type)?.label ?? tr.training_type}{tr.provider && ` — ${tr.provider}`}</p><div className="flex items-center gap-3 mt-1 text-xs text-theme-subtle"><span className="flex items-center gap-1"><Calendar className="w-3 h-3" aria-hidden="true" />{t('safeguarding.completed', 'Completed')}: {new Date(tr.completed_at).toLocaleDateString()}</span>{tr.expires_at && (<span>{t('safeguarding.expires', 'Expires')}: {new Date(tr.expires_at).toLocaleDateString()}</span>)}</div></div></div></GlassCard></motion.div>))}
        </motion.div>))}
