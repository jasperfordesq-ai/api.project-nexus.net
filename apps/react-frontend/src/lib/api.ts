...
/**
 * NEXUS API Client
 * Handles all HTTP communication with the ASP.NET Core backend
 *
 * Features:
...
//     connect-src 'self' https://api.project-nexus.net wss://api.project-nexus.net;
...
class ApiClient {
  ...
  /**
   * Normalize endpoint paths for ASP.NET Core backend compatibility.
   * The V1 PHP frontend used /v2/ prefixed endpoints; the ASP.NET backend
   * serves the same resources without the /v2/ prefix.
   */
  private normalizeEndpoint(endpoint: string): string {
    // Strip /v2/ prefix — ASP.NET backend serves at /api/* not /api/v2/*
    if (endpoint.startsWith('/v2/')) {
      return endpoint.substring(3); // '/v2/foo' → '/foo'
    }
    return endpoint;
  }
...
export async function checkBackendHealth(): Promise<{
  ...
  try {
    ...
    return {
      ...
      database: data.checks?.database === 'healthy',
    };
  } catch {
 ...
 * The ASP.NET Core backend uses JWT Bearer auth (not cookies), so CSRF
 * protection is not required. This function is kept as a no-op for
 * backwards compatibility with callers that invoke it on app init.
 */
export async function fetchCsrfToken(): Promise<string | null> {
  // No-op: ASP.NET Core backend uses JWT Bearer auth, CSRF not needed
  return null;
}
