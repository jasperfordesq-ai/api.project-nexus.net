 ...
 *
 * Updated for ASP.NET Core backend (fido2-net-lib):
 * - Registration: /passkeys/register/begin → /passkeys/register/finish
 * - Auth: /passkeys/authenticate/begin → /passkeys/authenticate/finish
 * - Management: GET /passkeys, DELETE /passkeys/{id}, PUT /passkeys/{id}
 */
...
const MOCK_CHALLENGE_DATA = {
  ...
  rp: { name: "Project NEXUS", id: "project-nexus.net" },
...
// ASP.NET backend returns { options, session_id } from authenticate/begin
const MOCK_AUTH_OPTIONS = {
  challenge: "auth-challenge-base64",
  rpId: "project-nexus.net",
  timeout: 60000,
...
// -------- Registration (ASP.NET backend: /passkeys/register/begin → finish)
...
describe("registerBiometric", () => {
  ...
  it("calls /passkeys/register/begin for challenge", async () => {
    mockApiPost.mockResolvedValueOnce({ success: true, data: MOCK_CHALLENGE_DATA }).mockResolvedValueOnce({ success: true });
    mockStartRegistration.mockResolvedValue(MOCK_CREDENTIAL);
    await registerBiometric("Custom Name", "platform");
    expect(mockApiPost.mock.calls[0][0]).toBe("/passkeys/register/begin");
  });

  it("passes displayName to /passkeys/register/finish", async () => {
    mockApiPost.mockResolvedValueOnce({ success: true, data: MOCK_CHALLENGE_DATA }).mockResolvedValueOnce({ success: true });
    mockStartRegistration.mockResolvedValue(MOCK_CREDENTIAL);
    await registerBiometric("Custom Name", "platform");
    expect(mockApiPost.mock.calls[1][0]).toBe("/passkeys/register/finish");
    expect(mockApiPost.mock.calls[1][1]).toMatchObject({ displayName: "Custom Name" });
  });
  ...
  it("uses default device name when none supplied", async () => {
    ...
    expect(mockApiPost.mock.calls[1][1].displayName).toBe("Windows PC");
  });
...
// -------- Authentication (ASP.NET: /passkeys/authenticate/begin → finish)
...
describe("authenticateWithBiometric", () => {
  ...
  it("returns success with user data on happy path", async () => {
    mockApiPost
      ...
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_RESULT });
  ...
  it("passes options from server to startAuthentication", async () => {
    mockApiPost
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_CHALLENGE })
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_RESULT });
    mockStartAuthentication.mockResolvedValue(MOCK_ASSERTION);
  ...
  it("sends sessionId and assertionResponse to /passkeys/authenticate/finish", async () => {
    mockApiPost
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_CHALLENGE })
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_RESULT });
    mockStartAuthentication.mockResolvedValue(MOCK_ASSERTION);
    await authenticateWithBiometric();
    expect(mockApiPost.mock.calls[1][0]).toBe("/passkeys/authenticate/finish");
    expect(mockApiPost.mock.calls[1][1]).toMatchObject({
      sessionId: "session-456",
      assertionResponse: MOCK_ASSERTION,
    });
  });
  ...
  it("returns error when verify step fails", async () => {
    mockApiPost
      .mockResolvedValueOnce({ success: true, data: MOCK_AUTH_CHALLENGE })
      .mockResolvedValueOnce({ success: false, error: "Signature invalid" });
    mockStartAuthentication.mockResolvedValue(MOCK_ASSERTION);
    const result = await authenticateWithBiometric();
...
// -------- Credential Management (ASP.NET: GET /passkeys, DELETE /passkeys/{id}, PUT /passkeys/{id})

describe("getWebAuthnStatus", () => {
  it("returns status derived from passkeys list", async () => {
    mockApiGet.mockResolvedValue({
      data: { passkeys: [{ id: 1 }, { id: 2 }] },
    });
    const result = await getWebAuthnStatus();
    expect(result).toEqual({ registered: true, count: 2 });
    expect(mockApiGet).toHaveBeenCalledWith("/passkeys");
  });

  it("returns not-registered when no passkeys", async () => {
    mockApiGet.mockResolvedValue({ data: { passkeys: [] } });
    const result = await getWebAuthnStatus();
...
describe("getWebAuthnCredentials", () => {
  it("returns passkeys array from API", async () => {
    const creds = [
      { id: 1, display_name: "Windows Hello", is_discoverable: true, transports: "internal", created_at: "2026-01-01T00:00:00Z", last_used_at: "2026-03-01T00:00:00Z" },
    ];
    mockApiGet.mockResolvedValue({ data: { passkeys: creds } });
  ...
  it("returns empty array when passkeys key missing", async () => {
...
describe("removeWebAuthnCredential", () => {
  it("returns true on success", async () => {
    ...
    expect(await removeWebAuthnCredential(1)).toBe(true);
    expect(mockApiDelete).toHaveBeenCalledWith("/passkeys/1");
  });

  it("returns false on failure", async () => {
    mockApiDelete.mockResolvedValue({ success: false });
    expect(await removeWebAuthnCredential(1)).toBe(false);
  });
});
...
describe("renameWebAuthnCredential", () => {
  it("returns true on success", async () => {
    ...
    expect(await renameWebAuthnCredential(1, "My YubiKey")).toBe(true);
    expect(mockApiPut).toHaveBeenCalledWith("/passkeys/1", { display_name: "My YubiKey" });
  });

  it("returns false on failure", async () => {
    mockApiPut.mockResolvedValue({ success: false });
    expect(await renameWebAuthnCredential(1, "My YubiKey")).toBe(false);
  });
});

describe("removeAllWebAuthnCredentials", () => {
  it("removes all credentials individually and returns count", async () => {
    mockApiGet.mockResolvedValue({
      data: { passkeys: [{ id: 1 }, { id: 2 }, { id: 3 }] },
    });
    mockApiDelete.mockResolvedValue({ success: true });
  ...
  it("returns 0 count when no credentials exist", async () => {
    mockApiGet.mockResolvedValue({ data: { passkeys: [] } });
    const result = await removeAllWebAuthnCredentials();
    expect(result).toEqual({ success: false, removedCount: 0 });
  });
