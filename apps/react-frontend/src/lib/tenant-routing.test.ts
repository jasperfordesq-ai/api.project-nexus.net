...
describe('tenant-routing', () => {
  ...
  describe('detectTenantFromUrl', () => {
    ...
    it('detects subdomain on project-nexus.net', () => {
      mockLocation('hour-timebank.project-nexus.net', '/dashboard');
      const result = detectTenantFromUrl();
      expect(result.slug).toBe('hour-timebank');
      expect(result.source).toBe('subdomain');
    });

    it('skips reserved subdomain (platform) and falls through to path', () => {
      mockLocation('platform.project-nexus.net', '/my-tenant/feed');
      const result = detectTenantFromUrl();
    ...
    it('returns null for reserved subdomain (api)', () => {
      mockLocation('api.project-nexus.net', '/');
      const result = detectTenantFromUrl();
      expect(result.slug).toBeNull();
      expect(result.source).toBeNull();
    });
    ...
    it('lowercases subdomain slugs', () => {
      mockLocation('HOUR-TIMEBANK.project-nexus.net', '/');
      const result = detectTenantFromUrl();
      expect(result.slug).toBe('hour-timebank');
    });

    it('rejects multi-level subdomains', () => {
      mockLocation('foo.bar.project-nexus.net', '/');
      const result = detectTenantFromUrl();
      expect(result.slug).toBeNull();
      expect(result.source).toBeNull();
    });
