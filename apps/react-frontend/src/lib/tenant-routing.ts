...
export const RESERVED_SUBDOMAINS = new Set([
  'app', 'platform', 'api', 'www', 'admin', 'mail', 'smtp', 'imap', 'pop', 'ftp',
 ...
 * Used for R3 (path-based resolution on platform.project-nexus.net).
 ...
 */
const PLATFORM_DOMAIN = 'project-nexus.net';
 ...
 * - R2: Subdomain of project-nexus.net → extract subdomain as slug
 * - R3: First path segment on platform.project-nexus.net → extract as slug if not reserved
...
export function detectTenantFromUrl(): { slug: string | null; source: 'subdomain' | 'path' | null } {
  ...
  // R2: Subdomain resolution — {sub}.project-nexus.net
  if (hostname.endsWith('.' + PLATFORM_DOMAIN)) {
    ...
    // Reserved subdomain (platform, app, api, etc.) — fall through to path-based
    if (subdomain === 'platform' || subdomain === 'app') {
      const slug = extractSlugFromPath(pathname);
      return { slug, source: slug ? 'path' : null };
    }
  ...
  // R1: Custom domain — not project-nexus.net
