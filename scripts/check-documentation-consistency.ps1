...
if ($null -ne $status) {
    Assert-Contains 'docs/CURRENT_ASPNET_CONTRACT_STATUS.md' $status `
        '<!--\s*doc-consistency:\s*ASPNET_CURRENT_BANKED_SCORE=680/1000\s*-->' `
        'must expose the canonical 680/1000 banked-score marker.'
    Assert-Contains 'docs/CURRENT_ASPNET_CONTRACT_STATUS.md' $status `
        '(?i)current banked score is\s+\*{0,2}680/1000' `
        'must state the current banked score as 680/1000.'
    Assert-Contains 'docs/CURRENT_ASPNET_CONTRACT_STATUS.md' $status `
        '2,601/2,601' `
...
foreach ($relativePath in $historicalOpenings) {
    ...
    Assert-NotContains $relativePath $opening `
        '(?i)\b(?:680/1000|622/1000|688/689|1,661/1,661)\b|663\s+contracts' `
...
foreach ($relativePath in $currentScoreDocuments) {
    ...
    foreach ($claim in $currentScoreClaims) {
        if ([int]$claim.Groups['points'].Value -ne 680) {
            Add-Failure "${relativePath}: contains a current overall score that contradicts 680/1000."
        }
    }
